...
fun RegisterScreen (navController: NavController, viewModel: RegisterViewModel, modifier: Modifier = Modifier) {
    ...
    val gender by viewModel.gender.observeAsState(Gender.MALE)
...
//        })
        GenderSelector(
            selected = gender,
            onSelected = { viewModel.onGenderUpdate(it) }
        )
...
fun GenderSelector(
    ...
    Column {
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            RadioButton(
                selected = selected == Gender.PREFER_NOT_TO_SAY,
                onClick = { onSelected(Gender.PREFER_NOT_TO_SAY) }
            )
